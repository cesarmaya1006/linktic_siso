...
<!-- ************************************************************* -->
@section('tituloHoja')
    Parametros - Responsables PC's Rentados
@endsection
<!-- ************************************************************* -->
        ...
        <div class="card-header">
            <div class="row mb-3">
                <div class="col-12 col-md-6 col-lg-6 text-md-left text-lg-left pl-2">
                    <h5>listado de Responsables PC's Rentados</h5>
                </div>
                <div class="col-12 col-md-6 col-lg-6 text-md-right text-lg-right pl-2 pr-md-5 pr-lg-5">
                    @if ($permiso==null || $permiso->guardar)
                    <a href="{{ route('admin-responsables-crear') }}"
                        class="btn btn-success btn-sm text-center pl-3 pr-3" style="font-size: 0.9em;"><i
                            class="fas fa-plus-circle mr-2"></i> Nuevo Responsable</a>
                    @endif
                </div>
            </div>
            <hr>
                    ...
                    <table class="table table-striped table-hover table-sm tabla_data_table_xs tabla-borrando" id="tabla-data">
                        ...
                        <tbody>
                            @foreach ($responsables as $responsable)
                                <tr>
                                    <td class="text-center">{{ $responsable->responsable }}</td>
                                    <td class="text-center">
                                        @if ($permiso == null || $permiso->actualizar == 1)
                                        <a href="{{ route('admin-responsables-editar', ['id' => $responsable->id]) }}"
                                            class="btn-accion-tabla tooltipsC text-info" title="Editar"><i
                                                class="fa fa-edit" aria-hidden="true"></i></a>
                                        @endif
                                        @if ($permiso == null || $permiso->borrar == 1)
                                        <form action="{{ route('admin-responsables-eliminar', ['id' => $responsable->id]) }}"
